...
namespace Insurance_Claim_Settlement_And_Audit_Pipeline_Project
{
    public static class ClaimCsvParser
    {
        public static Claim Parse(string line, int lineNumber)
        {
            ...
            {
                throw new ClaimException($"Line {lineNumber}: ClaimId cannot be empty.");
            }

            if (string.IsNullOrWhiteSpace(policyId))
            {
                throw new ClaimException($"Line {lineNumber}: PolicyId cannot be empty.");
            }

            if (string.IsNullOrWhiteSpace(claimType))
            {
                throw new ClaimException($"Line {lineNumber}: ClaimType cannot be empty.");
            }
            ...
            if (!DateTime.TryParseExact(dateText,"yyyy-MM-dd",CultureInfo.InvariantCulture,DateTimeStyles.None,out DateTime submittedDate))
            {
